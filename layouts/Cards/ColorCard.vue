<template>
  <div class="ccardl">
    ...
    <div
      ...
      :style="GetShadow() + `background-image: url(https://main-bloom-d-ltbdtk4j88hgmuxs-gtw.qovery.io/ren?width=900&height=500&method=cover&image=${album.thumb}); `"
      v-ripple>
    </div>
    <div
      class="ccard-sm"
      ...
      :style="`background: linear-gradient(289deg, ${hexToRgb(item.color[1], 0.4553571770505077)} 3%, ${hexToRgb(item.color[1], 0.010343171448266806)} 19%,   ${hexToRgb(item.color[0], 0.3699230033810399)} 80%, ${hexToRgb(item.color[0], 1)} 99%), url(https://main-bloom-d-ltbdtk4j88hgmuxs-gtw.qovery.io/ren?width=400&height=200&method=cover&image=${album.thumb}) right no-repeat; background-size: 19rem 10rem;`"
      v-ripple>
    </div>
    <div
      class="ccard-xs"
...
export default {
  ...
  methods: {
    hexToRgb(hex, opacity) {
      if (hex !== undefined) return 'rgba(' + (hex = hex.replace('#', '')).match(new RegExp('(.{' + hex.length / 3 + '})', 'g')).map(function(l) {
